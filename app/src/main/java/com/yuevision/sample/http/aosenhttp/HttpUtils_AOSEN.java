...
import com.yuevision.sample.http.HttpHead;
import com.yuevision.sample.http.NullOnEmptyConverterFactory;
import com.yuevision.sample.http.ParamNames;
...
public class HttpUtils_AOSEN {
    private static HttpUtils_AOSEN instance;
    private Gson gson;
    ...
    public static HttpUtils_AOSEN getInstance() {
        if (instance == null) {
            synchronized (HttpUtils_AOSEN.class) {
                if (instance == null) {
                    instance = new HttpUtils_AOSEN();
                }
            }
        }
        return instance;
     ...
     * @return
     */
    public <T> T getAosenServer(Class<T> clz) {
        if (https == null) {
            synchronized (HttpUtils_AOSEN.class) {
                https = getRetrofitBuilder(URLUtils.AOSEN_BASE_URL).build().create(clz);
            }
        }
        if (httpCacheDirectory == null || cache == null) {
            synchronized (HttpUtils_AOSEN.class) {
                httpCacheDirectory = new File(context.getCacheDir(), "efulai_acache_https");
                try {
                    cache = new Cache(httpCacheDirectory, 10 * 1024 * 1024);
                } catch (Exception e) {
                    Log.e("OKHttp", "Could not create http cache", e);
